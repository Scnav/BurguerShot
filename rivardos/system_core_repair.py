...
class DatabaseManager:
    ...
    def update_item(self, item_id, **kwargs):
        conn = self.get_connection()
        cursor = conn.cursor()
        allowed_fields = ['source_row', 'quantity', 'purchase_price', 'current_price', 'image_filename', 'steam_item_id', 'steam_market_url', 'steam_sale_fee', 'last_price_update']
...
class SteamManager:
    ...
    NAME_MAPPING = {
        "Cápsula de Adesivos das Lendas do Estocolmo 2021": "Stockholm 2021 Legends Sticker Capsule",
        "Cápsula de Autógrafos dos Campeões do Antuérpia 2022": "Antwerp 2022 Champions Autograph Capsule",
        "Caixa CS20": "CS20 Case",
        "Caixa do Ataque Ofídico": "Snakebite Case",
        "Caixa do Coice": "Recoil Case",
        "Caixa dos Sonhos e Pesadelos": "Dreams & Nightmares Case",
        "Caixa da Revolução": "Revolution Case",
        "Terminal Gênese Lacrado": "Sealed Graffiti | Genesis"
    }
    ...
    def search_item_on_steam(self, item_name):
        ...
        market_url = f"https://steamcommunity.com/market/listings/730/{steam_id}"
        if self.log_manager:
    ...
    def get_item_price(self, steam_id, currency_id=48):
        if not steam_id:
            return None
        ...
        if not price_str:
            if self.log_manager:
                self.log_manager.log_no_price(market_hash_name)
            return None
        cleaned = price_str.replace('R$', '').replace('R$ ', '').replace('$', '').replace('USD', '').strip()
        ...
        try:
            ...
            if self.log_manager:
                self.log_manager.log_error(f"Failed to parse price '{cleaned}'", f"get_item_price: {market_hash_name}")
            return None

    def get_last_sale_price(self, steam_id, currency_id=48):
        if not steam_id:
            return None
        market_hash_name = unquote(str(steam_id))
        params = {
    ...
    def is_item_data_mismatched(self, item_name, steam_id=None, steam_market_url=None):
        ...
        if steam_id:
            ...
            if self._normalize_market_name(decoded) != expected:
                return True
        if steam_market_url:
            url_id = urlparse(str(steam_market_url)).path.split('/market/listings/730/', 1)[-1]
